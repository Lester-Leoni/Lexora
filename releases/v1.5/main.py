...
class LexoraApp(DnDCTk):
    ...
    def format_time(self, seconds, decimal_separator=","):
        """Дублирующий точный каскад UI для сохранения сквозной консистентности."""
    ...
    def _export_srt(self, target_path: str):
        """Жесткий фикс кодировки: экспорт SRT в utf-8-sig (с BOM) для защиты от кракозябр на Windows."""
        try:
    ...
    def _export_vtt(self, target_path: str):
        """WebVTT строго специфицирован под стандартный utf-8 без BOM."""
        try:
            with open(target_path, "w", encoding="utf-8") as f:
                f.write("WEBVTT\n\n")
                for i, (start, end, text) in enumerate(self.transcribed_segments, start=1):
