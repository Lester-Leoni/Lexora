# Полный и цельный код app.py от первой до последней строчки без сокращений
import os
import sys
...
class LexoraApp(DnDCTk):
    def __init__(self):
        ...
        # Вариант 1: Элемент ctk.CTkOptionMenu для контролируемого ручного сохранения документов
    ...
    def format_time(self, seconds, decimal_separator=","):
        """Возвращает время в формате ЧЧ:ММ:СС,ммм или ЧЧ:ММ:СС.ммм в зависимости от сепаратора."""
        h = int(seconds // 3600)
    ...
    def _on_save_dropdown_click(self, choice: str):
        """Распределительный диспетчер выпадающего меню экспорта результатов."""
